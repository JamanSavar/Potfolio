...
public class Connection
{
    ...
    public Connection()
    {
        connection = new SqlConnection(connection_String(ConfigurationManager.ConnectionStrings["HR"].ToString()));            
    }
    ...
    public String connection_DB(string s_Quary, int i_QuaryState, bool b_IsReturnValue, bool b_Trans, bool b_Commit)
    {
        ...
        string s_ReaderType = String.Empty;
        ...
        try
        {
            ...
            else if (i_QuaryState == 1) // For Insert,Update,Delete
            {
                ...
                if (connection.State == ConnectionState.Closed && b_Trans == false)
                {
                    connection.Open();
                }
        ...
        catch (Exception)
        {
            try
            {
                ...
                if (b_Rollback == true && s_ReaderType=="Y")
                {
                    reader.Close();
                    transaction.Rollback();
                    transaction.Dispose();
                }
                else if (b_Rollback == true && s_ReaderType == "N")
                {
                    transaction.Rollback();
                    transaction.Dispose();
                }
    ...
    public void connection_DB_Image(int i_QuaryState, string s_LinkAutoId, byte[] b_Image)
    {
        ...
        try
        {
            connection = new SqlConnection(connection_String(ConfigurationManager.ConnectionStrings["DatabaseConnectionImage"].ToString()));
            if (i_QuaryState == 0)
            {
